...
#+BEGIN_SRC R :file charts/outliers.pdf
...
file_path <- "dataset/2025-04-DBEngineRanking.csv"

# Lire le fichier CSV
# Spécification du point-virgule comme séparateur
data <- read_delim(file_path, delim = ",")

# Afficher la structure des données
str(data)
  
# Filtrer les données pour la date du 2025-04-01
data_filtered <- data %>%
  filter(Date == "2025-04-01")

# On exclut la colonne de date pour ne garder que les valeurs des DBMS
...
# Créer le barcode plot
barcode_plot <- ggplot(data_long, aes(x = DBMS, y = 0)) +
  geom_point(aes(y = Value), shape = "|", size = 3) + 
  # Autre option : geom_point(aes(y = Value))
  labs(title = paste("Barcode Plot"),
       y = "Trend score",
       x = "DBMS") +
  theme_minimal() +
  theme(axis.text.y = element_blank(), # Supprime les graduations de l'axe Y
        axis.title.y = element_blank(), # Supprime le titre de l'axe Y
        axis.ticks.y = element_blank()) # Supprime les marques de l'axe Y
...
# Avec Postgresql comme DBMS il convient de se demander comment s'y prendre à la place de est-ce possible -- En conclusion, la question deviendrait "qu'est-ce qui est le plus performant" dont les critères de performances sont relatives à un cas d'étude.

# Note : ANSI/ISO SQL Compliance coverage of related DBMS
...
*** Géospatial
[[https://github.com/postgis/postgis][PostGIS]] est l'une des extensions de PostgreSQL les plus connues. Elle permet principalement de prendre en charge des modèles de données géographiques, géométriques et géospaciales.

# Usecase : CAO, BIM, CIM, etc. 
...
#+begin_quote
/Dans un monde plus ouvert qu’il n’a jamais été, notre Société traverse une nouvelle mutation en passant de l’économie industrielle à l’économie du service et de la connaissance./ -- Urban-Galindo[cite:@jean-jacquesurban-galindoGestionConfigurationIngenierie2008]
#+end_quote

La mise en place d’un environnement d’apprentissage, d’exploration et de partage de connaissance offre la capacité de développer son expertise. [cite:@vidicKnowledgeAssetCompetitive2022] Il s’agit d’une opportunité stratégique de long terme permettant le maintien des connaissances au sein d’une entreprise dans un contexte de raréfaction des ressources humaines,[cite:@thoumsinQuellesStrategiesFautil2019] lutant ainsi contre l’amnésie organisationnelle.[cite:@lucierivardCycleRetentionConnaissances2010]
...
| Nom           | Particularité                  |
|---------------+--------------------------------|
| [[https://github.com/brianc/node-postgres][node-postgres]] | Client pour Node.js            |
|               |                                |
|               |                                |
| [[https://github.com/porsager/postgres][postgres.js]]   | Client pour Node.js, Deno,     |
|               | Bun and CloudFlare             |
|               | Utilisé par TypeORM et Drizzle |
| [[https://github.com/gajus/slonik][slonik]]        | Client pour Node.js            |
|               | Type safety                    |
|               | Composable SQL                 |
...
| Nom                 | Particularité                         |
|---------------------+---------------------------------------|
| [[https://github.com/theory/pgtap][pgTAP]]               | Suite applicative pour                |
|                     | créer des tests unitaires             |
|                     |                                       |
| [[https://github.com/stack-auth/pgmock][pgmock]]              | In-memory PostgreSQL mock             |
|                     | server for unit and E2E tests         |
|                     |                                       |
| [[https://github.com/peterldowns/pgtestdb][pgtestdb]]            | Quickly run tests in their            |
|                     | own temporary, isolated,              |
|                     | databases                             |
| [[https://github.com/bytebase/bytebase][bytebase]]            | DevSecOps solution for Developer,     |
|                     | Security, DBA and Platform            |
|                     | Engineering teams                     |
| [[https://github.com/postgres-ai/database-lab-engine][database-lab-engine]] | Enables database branching and        |
...
| Nom              | Particularité                           |
|------------------+-----------------------------------------|
| [[https://github.com/pgaudit/pgaudit][pgaudit]]          | SOC2                                    |
|                  |                                         |
|                  |                                         |
| [[https://github.com/pgsentinel/pgsentinel][pgsentinel]]       | Active session history                  |
|                  |                                         |
|                  |                                         |
| [[https://github.com/paradedb/pg_analytics][pg_analytics]]     | Requeter des stockages comme AWS3 et    |
...
|                  |                                         |
|                  |                                         |
| [[https://github.com/powa-team/pg_qualstats/][pg_qualstats]]     | Collecte des statistiques sur           |
|                  | les prédicats et aide à                 |
|                  | trouver les indices manquants           |
| [[https://github.com/postgrespro/pg_query_state][pg_query_state]]   | Requeter le moniteur d'avancement       |
|                  |                                         |
|                  |                                         |
| [[https://github.com/powa-team/pg_stat_kcache/][pg_stat_kcache]]   | Regroupe les statistiques sur           |
|                  | les lectures et écritures               |
|                  | effectués par le système de fichiers    |
| [[https://github.com/percona/pg_stat_monitor][pg_stat_monitor]]  | Analyse des performances                |
|                  |                                         |
|                  |                                         |
| [[https://github.com/postgrespro/pg_wait_sampling][pg_wait_sampling]] | Sampling based statistics of            |
|                  | wait events                             |
|                  |                                         |
| [[https://github.com/powa-team/powa][PoWA]]             | Workload Analyzer                       |
|                  |                                         |
|                  |                                         |
| [[https://codeberg.org/Data-Bene/StatsMgr][StatsMgr]]         | Gestion des statistiques de             |
|                  | performance et d'utilisation            |
|                  |                                         |
| [[https://github.com/EnterpriseDB/system_stats][system_stats]]     | Expose les données du système comme     |
|                  | l'utilisation du CPU,                   |
|                  | de la mémoire et des disques            |
| [[https://github.com/dalibo/pg_activity][pg_activity]]      | Server activity monitoring              |
|                  |                                         |
|                  |                                         |
| [[https://github.com/EnterpriseDB/pg_catcheck][pg_catcheck]]      | Outil pour diagnostiquer la             |
|                  | corruption du catalogue système         |
|                  |                                         |
| [[https://github.com/df7cb/pg_dirtyread][pg_dirtyread]]     | Read dead but unvacuumed tuples         |
|                  | from a PostgreSQL relation              |
|                  |                                         |
| [[https://github.com/cybertec-postgresql/pg_show_plans][pg_show_plans]]    | Show query plans of all                 |
|                  | currently running                       |
|                  | SQL statements                          |
| [[https://github.com/nexsol-technologies/pgassistant][pgassistant]]      | Tool designed to help                   |
|                  | understand and                          |
|                  | optimize performances                   |
| [[https://github.com/CrunchyData/pgmonitor-extension][pgmonitor]]        | Suite de surveillance des               |
|                  | performances et de l'état               |
|                  | des bases de données                    |
| [[https://github.com/bigsql/plprofiler][plprofiler]]       | Permet de créer des profils de          |
|                  | performance pour                        |
|                  | les procédures stockées                 |
| [[https://github.com/tvondra/tdigest][tdigest]]          | Estimation des  percentiles             |
|                  | avec T-Digest                           |
|                  |                                         |
| [[https://gitlab.com/postgres-ai/joe][Joe]]              | (AI tool) to experiment on              |
...
MSSQL, propriétaire : 
MySQL, Version communautaire GPL-2.0[fn:4] sinon propriétaire : Parallel Query + CTE vs limitations replication


# PostgreSQL face aux SGBD NoSQL et NewSQL
MongoDB, licence "source-accessible" SSPL[fn:3] : Flexibilité schéma (JSONB) + ACID vs AP de MongoDB

# Compétiteurs émergentes : SurrealDB, DuckDB
# Discuter de la menace de SurrealDB (all-in-one) et de DuckDB (OLAP embarqué).

- SurrealDB, licence "source-accessible" BSL[fn:5] : All-in-one vs modularité PostgreSQL
- SpacetimeDB, licence "source-accessible" BSL[fn:5] : +Blockchain+ Stored Procedures vs maturité écosystème
- DuckDB, licence open source MIT[fn:2] : OLAP embarqué vs scalabilité Citus
- ElasticSearch,  Elastic License[fn:6]; AGPL[fn:7]; SSPL[fn:5] : vs ParadeDB
...
* Footnotes
# A transformer en définitions et créer un sBOM depuis postgresql
[fn:1] PostgreSQL License : [[https://spdx.org/licenses/PostgreSQL.html]]
[fn:2] MIT Licence : [[https://spdx.org/licenses/MIT.html]]
[fn:3] Server Side Public Licence 1.0 : [[https://spdx.org/licenses/preview/SSPL-1.0.html]]
[fn:4] GNU Public Licenve 2.0 : [[https://spdx.org/licenses/GPL-2.0-only.html]]
[fn:5] Business Source License 1.1 : [[https://spdx.org/licenses/BUSL-1.1.html]] 
[fn:6] Elastic License : [[https://spdx.org/licenses/Elastic-2.0.html]]
[fn:7] Affero GNU Public License 3.0 : [[https://spdx.org/licenses/AGPL-3.0-only.html]]
